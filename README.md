...
--------- | ---------
config_directory_path | The path to the output directory of the file generated by the `bin/rails generate active_record:dbt:*` command. Default is `lib/dbt`.
...
Generate `#{config_directory_path}/source_config.yml`.

```bash
$ bin/rails generate active_record:dbt:config
```

In the `#{config_directory_path}/source_config.yml` file, describe the items you want to set for the source properties.
We can set `sources`, `table_overrides`, `defaults` and `table_descriptions` in this file.

The items that can be set for `sources` and `table_overrides` are described in [Source properties | dbt Developer Hub](https://docs.getdbt.com/reference/source-properties).
